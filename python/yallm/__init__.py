...
class YaLLMNotFound(FileNotFoundError): ...


def find_yallm_bin() -> str:
    """Return the yallm binary path."""

    yallm_exe = "yallm" + sysconfig.get_config_var("EXE")
    ...
    targets = [
        ...
        (
            # On Windows, with module path `<prefix>/Lib/site-packages/yallm`
            _join(
                _matching_parents(_module_path(), "Lib/site-packages/yallm"), "Scripts"
            )
            if sys.platform == "win32"
            # On Unix,  with module path `<prefix>/lib/python3.13/site-packages/yallm`
            else _join(
                _matching_parents(_module_path(), "lib/python*/site-packages/yallm"),
                "bin",
            )
        ),
        # Adjacent to the package root, e.g., from `pip install --target`
        # with module path `<target>/yallm`
        _join(_matching_parents(_module_path(), "yallm"), "bin"),
    ...
    for target in targets:
        ...
        if target in seen:
            continue
        seen.append(target)
        path = os.path.join(target, yallm_exe)
        if os.path.isfile(path):
            return path

    locations = "\n".join(f" - {target}" for target in seen)
    raise YaLLMNotFound(
        f"Could not find the yallm binary in any of the following locations:\n{locations}\n"
    )
...
def _user_scheme() -> str:
    if sys.version_info >= (3, 10):
        ...
        user_scheme = "nt_user"
    elif sys.platform == "darwin" and getattr(sys, "_framework", False):
